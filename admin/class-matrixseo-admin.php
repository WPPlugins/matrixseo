...
class MatrixSeo_Admin {
	...
	public function matrixseo_page(){
		...
		if($_SERVER['REQUEST_METHOD'] === 'POST'){
			...
			if( isset( $_POST['ips'] ) && $_POST['allow_edit_ips']=="on" ){
			...
			if( isset( $_POST['referers'] ) && $_POST['allow_edit_refs']=="on" ){
        ...
        if($tab == 'actions'){
            $results = $wpdb->get_results("	
											...
											INNER JOIN ".$wpdb->prefix."mx_seo_actions as actions ON urls.url = actions.hash
											LEFT JOIN ".$wpdb->prefix."mx_seo_ignore as ignr ON ignr.id_url = urls.id
											WHERE actions.action_id <> 2
											AND ignr.id_url IS NULL
											LIMIT 10" ,ARRAY_A);
		...
		if(isset($_GET['searchurl'])){
			if( isset($_GET['searchurl']) && !empty($_GET['searchurl']) ){
				...
				$search_result = $wpdb->get_results($wpdb->prepare("SELECT urls.id as id_website, actions.action_id, actions.data, urls.* FROM ".$wpdb->prefix."mx_seo_urls as urls
