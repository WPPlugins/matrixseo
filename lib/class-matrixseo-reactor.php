...
class MatrixSeo_Reactor {
    ...
    /**
     * This function gets the content requests from files and returns them in an array.
     * @since   1.0.0
     * @access  private
     * @param   bool    $deleteAfterCall    Delete after call
     * @return  array   The actions files content as array.
     */
/*    private static function getContentRequestsAsArray($deleteAfterCall = true){
        $contents = array();
        if(MatrixSeo_Config::get("mx_save_page_content")=="0"){
            return $contents;
        }
        MatrixSeo_Utils::cronDebug("Getting content requests as array...",3);
        global $wpdb;
        $results = $wpdb->get_results($wpdb->prepare("SELECT hash FROM " . $wpdb->prefix . "mx_seo_actions WHERE action_id = %d", 2), ARRAY_A);
        foreach ($results as $result) {
            $path = MatrixSeo_Utils::getUrlsDirectory(MatrixSeo_Utils::getPartialPath($result['hash']));
            if (file_exists($path)) {
                $contents[$result['hash']] = MatrixSeo_Utils::getSafeFileContents($path);
                if ($deleteAfterCall) {
                    MatrixSeo_Utils::$deleteFilesQueue[] = $path; //mark file for deletion
                    MatrixSeo_Utils::$deleteSendActionsFromDBQueue[] = "'" . $result['hash'] . "'"; //mark db row for deletion
                }
            }
        }
        MatrixSeo_Utils::cronDebug("Content sent", 1);
        return $contents;
    }*/
    ...
    private static function setSearchEngineToFile($html = ''){
        ...
        $data = array(
            ...
            "title"     => implode(' ', MatrixSeo_Utils::$currentTitle),
    ...
    /**
     * This function is deleting the actions from database.
     * @since   1.0.0
     * @access  private
     * @param   void
     * @return  bool
     */
/*    private static function processDeleteSendActionFromDB(){
        MatrixSeo_Utils::cronDebug("Deleting actions marked for deletion from DB...", 3);
        if (count(MatrixSeo_Utils::$deleteSendActionsFromDBQueue) == 0) {
            MatrixSeo_Utils::cronDebug("No action marked for deletion from DB.", 2);
            return false;
        }
        global $wpdb;
        $query = "DELETE FROM " . $wpdb->prefix . "mx_seo_actions WHERE `hash` IN (" . implode(',', MatrixSeo_Utils::$deleteSendActionsFromDBQueue) . ")";
        $wpdb->query($query);
        MatrixSeo_Utils::$deleteSendActionsFromDBQueue = array();
        MatrixSeo_Utils::cronDebug("Action marked for deletion deleted from DB.", 2);
        return true;
    }*/
    ...
    private static function getStrippedPowerWords($html){
        ...
        $titleWords     = explode(" ",implode(" ",MatrixSeo_Utils::$currentTitle));
    ...
    public function detectAndSaveVisitor(){
        ...
        if(
            ...
            if($this->detectSearchEngineByIP()){
                ...
                ob_start( array('MatrixSeo_Reactor', 'setHtmlToFile') );
            }
    ...
    /**
     * This function handles the change of title. The payload is already stored into $this->rules[1].
     * @since   1.0.0
     * @acces   public
     * @param   array   $title  The title received from WordPress
     * @return  mixed   Changed title array
     */
    public function handleChangeTitleParts($title){
        if(self::$react) {
            global $wpdb;
            $currentURL = MatrixSeo_Utils::getFullUrl();

            $newTitle = implode(' &#8211; ', MatrixSeo_Utils::$currentTitle); //TODO: Use a function to get the separator instead of '-'
            ...
            if (isset($ignore) &&
                ...
                $newTitle .= stripslashes(MatrixSeo_Config::get('mx_beginning_title_separator')) .
                    $rules[1] .
                    stripslashes(MatrixSeo_Config::get('mx_end_title_separator'));
                $title['title'] = $newTitle;
                $title['page'] = '';
                $title['tagline'] = '';
                $title['site'] = '';

                $implodedTitle = trim(implode(" ", $title));
                self::$newTitle = $implodedTitle;
                MatrixSeo_Utils::cronDebug("Changed title for [ " . $currentURL . " ] to " . $implodedTitle, 1);
                return $title;
            }
            return $title;
        }
    ...
    /**
     * This function handles the change of title through PRE method. The payload is already stored into $this->rules[1].
     * @since   1.0.0
     * @acces   public
     * @param   string $title  The title received from WordPress
     * @return  string Changed title
     */
    public function handleChangeTitlePre($title){
        if(self::$react) {
            global $wpdb;
            $currentURL = MatrixSeo_Utils::getFullUrl();
            if (count(MatrixSeo_Utils::$currentTitle)) {
                $newTitle = implode(" &#8211; ", MatrixSeo_Utils::$currentTitle);
                ...
                MatrixSeo_Utils::cronDebug("Changed title for [ " . $currentURL . " ] to " . $newTitle, 1);
                return $newTitle;
            }
        }
    }
    ...
    /**
     * This funtion handles the change of title through wp_title method. The payload is already stored into $this->rules[1].
     * @since   1.0.0
     * @acces   public
     * @param   string  $title  The title received from WordPress
     * @param   string  $sep    The separator received from WordPress
     * @return  string  The new title as string
     */
    public function handleChangeTitleTitle($title, $sep){
        if(self::$react) {
            MatrixSeo_Utils::cronDebug("CHANGING wp_title");
            global $wpdb;
            $currentURL = MatrixSeo_Utils::getFullUrl();
            if (count(MatrixSeo_Utils::$currentTitle)) {
                $newTitle = implode(" &#8211; ", MatrixSeo_Utils::$currentTitle);
    ...
    /**
     * This function gets Current page title PRE method.
     * @since   1.0.0
     * @access  public
     * @param   string  $title  Title received from WordPress
     * @return  void
     */
    public function getHookPageTitlePre($title){
        if(self::$react) {
            if (!count(MatrixSeo_Utils::$currentTitle)) {
                if ($title != '') {
                    MatrixSeo_Utils::cronDebug("Getting (pre) title [ " . var_export($title, true) . " ]", 2);

                    $rules = self::getRulesFromFile(MatrixSeo_Utils::getFullUrl());
                    if (isset($rules[1])) {
                        MatrixSeo_Utils::$currentTitle['title'] = str_replace($rules[1], '', $title);
                    } else {
                        MatrixSeo_Utils::$currentTitle['title'] = $title;
                    }
                }
            }
        }
    ...
    /**
     * This function gets the Current page title Parts method.
     * @since   1.0.0
     * @access  public
     * @param   array   $title  Current page title elements received from WordPress
     * @return  mixed
     */
    public function getHookPageTitleParts($title){
        if(self::$react) {
            MatrixSeo_Utils::cronDebug("Getting (parts) title for [ " . MatrixSeo_Utils::getFullUrl() . " ] : [ " . implode(" ", $title) . " ]", 2);
            if (empty(MatrixSeo_Utils::$currentTitle)) {
                MatrixSeo_Utils::$currentTitle = $title;
                return $title;
            }
        }
    }
    ...
    /**
     * This function gets the Current page title.
     * @since   1.0.0
     * @acces   public
     * @param   string      $title  Current page title elements from WordPress
     * @param   string      $sep    Current page title separator from WordPress
     * @return  void
     */
    public function getHookPageTitleTitle($title,$sep){
        if(self::$react) {
            MatrixSeo_Utils::cronDebug("GETTING wp_title");

            if (!count(MatrixSeo_Utils::$currentTitle)) {
                MatrixSeo_Utils::cronDebug("Getting (wp_title) title [ " . $title . " ]", 2);
                if (empty(MatrixSeo_Utils::$currentTitle)) {
                    MatrixSeo_Utils::$currentTitle['title'] = $title;
                }
            }
    ...
    public function setHtmlToFile($html = ''){
        ...
        if( !defined( 'DOING_CRON' )                        &&
            strpos($currentURL, 'wp-cron.php') === false
        ){
            MatrixSeo_Utils::cronDebug("Getting HTML content of page...",3);
            $title = '';
            if(is_array(MatrixSeo_Utils::$currentTitle)){
                $title = implode(' ', MatrixSeo_Utils::$currentTitle);
            }
            $data = array(
                'title' => $title,
    ...
    public function send_data(){
        ...
        if(
            ...
            //self::processDeleteSendActionFromDB();
    ...
    public static function getAPIStopWordsList(){
        ...
        $api        =   MatrixSeo_Api::getInstance();
        $response   =   $api->call('refresh-stop-words',array('checksum'=>MatrixSeo_Utils::getStopWordsCheckSum()));
        if  (
            (is_array($response))                   &&
            (isset($response['ok']))                &&
            ($response['ok']==true)                 &&
            isset($response['checksum_validated'])
        ){
	...
	/*
	 * ===          FALLBACK 4          ===
	 * ALL IN ONE SEO PACK PLUGIN CHANGES THE TITLE USING OB_START
	 * WE ARE FORCED TO IMPLEMENT THIS FINAL FALLBACK METHOD ON CHANGING THE TITLE
	 ...
	 * @return  string  mixed
	 */
    public static function replaceTitleFallback( $content, $title='' ) {
    	if($title!='') {
		    $newContent = preg_replace( '/<title([^>]*?)\s*>([^<]*?)<\/title\s*>/is', '<title\\1>' . preg_replace( '/(\$|\\\\)(?=\d)/', '\\\\\1', strip_tags( $title ) ) . '</title>', $content, 1 );
		    if ( $content != $newContent ) {
			    MatrixSeo_Utils::cronDebug( "Force changing title...", 1 );
		    }
	    }
	    else{
    		$newContent=$content;
		...
		if(MatrixSeo_Config::get("mx_signature_active")==="1") {
			$newContent = preg_replace( '/' . preg_quote( '</title>', '/' ) . '/', "</title>\n<!-- Website enhanced by https://www.MatrixSEO.ai/ -->", $newContent, 1 );
		}
	    return $newContent;
	}
	 ...
	 * @return  string
	 */
	public static function rewriteTitleFallback( $header ) {
		$title = self::$newTitle;
		$header = self::replaceTitleFallback( $header, $title );
		return $header;
	}
	 ...
	 * @return  string
	 */
	public static function outputCallbackForTitleFallback( $content ) {
		return self::rewriteTitleFallback( $content );
	}
	...
	public static function template_redirect(){
		ob_start( array( 'MatrixSeo_Reactor', 'outputCallbackForTitleFallback' ) );
	}
