...
class MatrixSeo_Utils
{
     ...
     * @access  public
     */
    public static $currentTitle                 =   array();
     ...
     * @since   1.0.0
     * @access  public
     * @param   boolean     $showRequest    if false show home url
     ...
     * @since   1.0.0
     * @access  public
     * @param   string  $what   the data to write in the debug log
     * @param   int     $level  level of debug 1 < 2 < 3
     * @param   string  $filename   -> since 1.0.5
     * @return void
     */
    public static function cronDebug($what, $level=1, $filename="debug.php"){
        if(
                MatrixSeo_Config::get('mx_activate_cronlog') == '1' ||
                MatrixSeo_Config::get('mx_activate_cronlog') === false
          ){        	
    ...
    public static function getSearchEngineIPsFromFile(){
        ...
        self::cronDebug("Got search engines IPs from file", 1);
        return explode("\n", $ipList);
    }
    ...
    public static function getReferrerMatchesFromFile(){
        ...
        self::cronDebug("Got referrer fingerprints from file", 1);
        return explode("\n", $refList);
    }
    ...
    public static function isIpInRange($ip, $range ) {
        //TODO: rewrite this with inet_pton for ipv6
        if ( strpos( $range, '-' ) ){
     ...
     * @param	string  $ip     The ip that visited the page
     * @param	array   $extra  Data to be written to file
     * @return  void
     */
     ...
     * @since   1.0.0
     * @access  public
     * @param	string  $full   The full path of the file
     ...
     * @param 	boolean $append     True - Append, False - override
     * @return  void
     */
     ...
     * @param 	string 		$target_encoding The target encoding type
     ...
     * @since   1.0.5
     * @access  public
     * @param   $full_path, $log_row, $max_file_size
     * @return  void
     */
    ...
    public static function buildLogRow($row_data){
    	...
    	if(!empty($row_data))
    	{
    		...
    		{
    			if($field == 'ip') $value =  self::sanitizeIp($value);
    			elseif($field == 'date') $value =  self::sanitizeDate($value);
    			elseif($field == 'url') $value =  self::sanitizeUrl($value);
    			elseif($field == 'referrer') $value =  self::sanitizeReferrer($value);
    			elseif($field == 'title') $value =  self::sanitizeTitle($value);
    			elseif($field == 'words') $value =  self::sanitizeWords($value);
	 ...
	 * @return  array
	 */
    private static function getContentRequestsAsArrayNew()
    {
    	global $wpdb;
    ...
    public static function buildSyncDataPackage($max_data_size = 8000000){
    	...
    	$all_files = array(
    		's_files' => array_diff(glob(self::getSearchEnginesDirectory('*.php')),array(MatrixSeo_Utils::getSearchEnginesDirectory("index.php"))),
    		'r_files' => array_diff(glob(self::getReferrersDirectory('*.php')),array(MatrixSeo_Utils::getReferrersDirectory("index.php"))),
    		'contents' => self::getContentRequestsAsArrayNew()
    		);
    	...
    	return array($package_files, $delete_files_queue);
    }
